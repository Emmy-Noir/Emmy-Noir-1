  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-border/70 bg-background/80 backdrop-blur-md">
      ...
      >
        <div
          className={cn(
            'absolute inset-0 bg-noir/60 backdrop-blur-sm transition-opacity duration-300',
        ...
        <div
          className={cn(
            'absolute left-0 top-0 h-full w-[82%] max-w-sm overflow-y-auto bg-background p-6 shadow-2xl transition-transform duration-300 ease-editorial',
          ...
          <nav className="mt-10 flex flex-col gap-1">
            {mainNav.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={() => setOpen(false)}
                className="border-b border-border/60 py-4 font-serif text-2xl tracking-tight text-foreground"
              >
                {item.label}
              </Link>
            ))}
